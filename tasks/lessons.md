...
**规则**：本项目改滑块默认值必须同时动四处，缺一处就会不一致：

1. `state` 的初始值（真正生效的那个）
2. `buildShareUrl()` 的默认值判断（否则默认值被当成非默认写进链接）
3. `applyUrlParams()` 的 `clampNum` 范围（改了 `min`/`max` 才要动）
4. `llms.txt` 对外文档

`index.html` 的 `value` 只是首帧渲染前的占位，改它是给人看的，不是给程序看的。

**更一般的教训**：引入「单一数据源回写 UI」的机制时，要同步更新文档说明哪些地方从此不再是数据源——否则下次改的人（包括自己）会对着失效的入口调半天。
...
**踩坑**（2026-08-24，背景库扩容）：写了「1080 宽上限 + 质量自适应到 ≤200KB」的压缩函数批量处理，结果 `photo-snow-peaks.jpg`（413 KB / 1080×1440）在 q82 下重编码涨到 446 KB。源图本身已经压得很紧时，重编码只是又一次有损压缩加一次编码器差异，净效果可能是变大。

**规则**：任何批量重编码都要以「取更小者」收尾，别假设压缩必然变小：

```python
return out if len(out) < len(raw) else raw
```

同时**只在超出上限时缩放，绝不放大**（`if w > MAX_W`）——源图 960 宽被放到 1080 是纯粹的体积浪费加模糊。

**连带教训**：规划阶段的体积估算要实测。我口头估「压缩后 8–12 MB」，实测 17.9 MB，差了一倍——原因就是源图已压紧、压缩收益远低于直觉。给数字前先在内存里跑一遍样本。

---

## .gitignore 里不带斜杠的模式匹配任意层级

**规则**（2026-08-24）：`.gitignore` 写 `bg_*.jpg`（无路径分隔符）会匹配**仓库任意目录下**的同名模式，不只是根目录。本项目新图库前缀定为 `bj_`，和这条规则只差一个字母——手误写成 `bg_` 会让 142 张图被 git 静默忽略，本地一切正常、部署后全部 404，而且 `git status` 干净得看不出问题。

**规则**：批量新增资源文件前，先验一次是否被忽略：

```bash
git check-ignore -v backgrounds/bj_1-ibigqiang.jpg   # 无输出 = 未被忽略
```

「本地能跑、线上 404」这类故障优先怀疑 gitignore，不是服务器配置。

---

## 幂等全量渲染的函数里不能藏一次性副作用

**踩坑**（2026-08-24，背景网格改造）：`renderBackgroundGrid()` 原来兼着「渲染第一张时如果 `state.bg` 为空就设为默认背景」。原先它一辈子只跑一次，所以看不出问题；改成会因搜索/展开反复重渲染之后，这个副作用就变成「每次搜索都把用户选的背景重置回第一张」。同一次改造还发现 `setBg()` 靠传入的 DOM 元素打 `active` 标记，重渲染后元素被销毁、选中态跟着丢。

**规则**：把「只渲染一次」的函数改成「会反复重渲染」之前，先通读它找两类东西——**初始化副作用**（挪到 `init()`）和**对 DOM 元素引用的持有**（改成从 state 派生、渲染时比对）。这类耦合不会报错，只会静默地功能退化，评审代码时也很容易看漏，因为它在原来的调用次数下是正确的。

---

## PowerShell 的 -like 里 `?` 是通配符

**踩坑**（2026-08-24）：用 `git status --short | Where-Object { $_ -like '??*' }` 统计未跟踪文件，得到「164 行全是 ??」，与「刚做了 23 次 `git mv` 应该有 R」的预期矛盾，一度怀疑重命名没被识别。实际是 `?` 在 `-like` 中是**单字符通配符**，`'??*'` 匹配了每一行。

**规则**：统计 `git status --short` 的状态分布用取子串分组，别用通配符匹配：

```powershell
git status --short | Group-Object { $_.Substring(0,2) } | Select-Object Name, Count
```

更一般地：得到「所有项都落在同一类」这种结果时，先怀疑判据本身失效，别急着怀疑被统计的对象。

---

## 同一套编号的两端都手写，就等着它们错开

**踩坑**（2026-08-24，背景库编号）：`gen_backgrounds.py` 里 travel 段是 `bj_{32 + i}`（110 张，占到 142），渐变段是 `SVG_BASE = 142`。两个数字属于同一套连续编号，却互不引用——往 `PHOTOS` 末尾加一张具名图，它会拿到 143，和第一个渐变撞号。而撞号**不报错**：`numbered()` 照样产出两个「xxx 143」，前端搜索和分享链接跟着一起错。

**规则**：一套连续编号只允许有一个手写起点，后面的段从前面派生：

```python
SVG_BASE = max(int(NUM_RE.match(fn).group(1)) for fn, _, _ in PHOTOS)
```

派生不了的（比如序号写死在文件名里）就加断言，别指望下次改的人记得。生成物脚本的断言要拦「静默产出错误结果」的那类错，不是拦崩溃——崩溃自己会喊。

---

## 给一个来历不明的值补注释，不要编「为什么」

**踩坑**（2026-08-24，代码审查要求给 `.sg-label` 的不对称 padding 补注释）：`padding: 4px 8px 5px 8px`，下比上多 1px。我顺手写了「13px 中文在 line-height:1 下基线以下没余量，补 1px 才视觉居中」——听起来很专业。实测 `actualBoundingBox`：墨迹上余 0.4px、下余 0.6px，**偏差 0.2px 且方向相反**，字形上根本找不出依据。那 1px 是上一轮盯着屏幕手调的。

编出来的理由比没有注释更糟：它会让下一个人（包括自己）基于一个假前提去推算、去「修正」。

**规则**：补注释时如果不知道某个值的来历，就写它的来历不明加实测结论，别补一个自洽的故事。「这是手调的，别照着算式推」是有效信息；错误的推导过程是负资产。判断方法：写完注释问一句「这个解释我能验证吗」——不能就改成如实记录。
